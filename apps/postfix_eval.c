...
int eval_postfix(const char *expr, long *res)
{
    ...
    if (strlen(expr)==0 || res==NULL) {
        perror("Error: malformed expression. Too few arguments");
        return 0;
    }
    ...
    while (*expr != '\0')
    {
        if (isdigit(*expr) && *expr>=0)
        {
            num = malloc(sizeof(long int));
            sscanf(expr, "%ld", num);
            upo_stack_push(stack, (void*)num);
        }
        else if (ispunct(*expr))
        {
            ...
            switch (*expr) {
                case '+':
                    ...
                    *result += *(long int*) upo_stack_top(stack);
                    upo_stack_pop(stack, 1);
                    upo_stack_push(stack, (void*)result);
                    break;
                case '-':
                    *result = *(long int*) upo_stack_top(stack);
                    upo_stack_pop(stack, 1);
